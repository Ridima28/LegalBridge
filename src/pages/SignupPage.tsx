...
import { Scale, Mail, Lock, Eye, EyeOff, User, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const getStrengthLabel = () => {
    ...
    if (strength <= 2) return { label: 'Weak', color: 'text-error-500 bg-error-100' };
    if (strength <= 3) return { label: 'Medium', color: 'text-warning-500 bg-warning-100' };
    if (strength <= 4) return { label: 'Strong', color: 'text-success-500 bg-success-100' };
    return { label: 'Very Strong', color: 'text-success-600 bg-success-100' };
  };
    ...
    <div className="min-h-screen gradient-bg flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full">
        <div className="card p-8">
          ...
          {/* Error */}
          {error && (
            <div className="flex items-center gap-2 p-4 bg-error-50 text-error-700 rounded-xl mb-6">
                  ...
                  <div className="flex-1 h-1.5 bg-secondary-100 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-300 ${
                        passwordStrength() <= 2 ? 'bg-error-500' :
                        passwordStrength() <= 3 ? 'bg-warning-500' : 'bg-success-500'
                      }`}
                      style={{ width: `${(passwordStrength() / 5) * 100}%` }}
                    />
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${getStrengthLabel().color}`}>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center">
              <span className="px-4 bg-white text-secondary-500 text-sm">Or continue with</span>
            </div>
          </div>
